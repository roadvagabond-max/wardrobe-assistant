 ...
 * @param {number} options.timeoutMs default 15000 (15 seconds max)
...
export async function removeImageBackground(imageSource, options = {}) {
  ...
  try {
    const segmentationPromise = (async () => {
      ...
      const rawMaskedBlob = await removeBackground(imageSource, config);
    ...
    console.warn('Háttéreltávolítási figyelmeztetés / fallback:', err);
    return {
      success: false,
...
export async function processGarmentPackshot(fileOrDataUrl, options = {}) {
  try {
    ...
    console.warn('processGarmentPackshot hiba:', err);
    return {
      success: false,
      error: err.message
    };
