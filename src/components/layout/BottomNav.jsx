...
export default function BottomNav({ activeTab, setActiveTab }) {
  ...
  return (
    <div className="fixed bottom-3 sm:bottom-5 left-0 right-0 z-40 flex justify-center px-3 pointer-events-none">
      <nav className="pointer-events-auto bg-[#1a1714]/92 backdrop-blur-2xl border border-[var(--border-gold)]/45 rounded-full p-1.5 sm:p-2 shadow-[0_12px_40px_rgba(0,0,0,0.7),0_0_24px_var(--accent-gold-glow)] flex items-center gap-1 sm:gap-2">
        {tabs.map(tab => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 sm:gap-2 py-2 px-3 sm:px-4 rounded-full transition-all duration-300 relative select-none ${
                isActive 
                  ? 'bg-[var(--accent-gold)] text-[#12100e] font-bold shadow-md shadow-[var(--accent-gold)]/30 scale-102' 
